...
const API_KEY = process.env.NEXT_PUBLIC_ALGOLIA_ADMIN_API_KEY ?? process.env.ALGOLIA_ADMIN_API_KEY
const INDEX_NAME = process.env.NEXT_PUBLIC_ALGOLIA_INDEX_NAME ?? process.env.ALGOLIA_INDEX_NAME
...
if (!APP_ID || !API_KEY || !INDEX_NAME) {
  ...
  console.error("- NEXT_PUBLIC_ALGOLIA_ADMIN_API_KEY or ALGOLIA_ADMIN_API_KEY (for indexing)")
...
async function getAllContentForIndex() {
  ...
  const pageRecords = pages.map(page => {
    ...
    const record = {
      ...
      title,
      type: "cms_page",
      description: page.description ? extractTextFromSanityObject(page.description) : undefined,
  ...
  const scheduleRecords = scheduleItems.map(item => {
    ...
    let description = item.description ? extractTextFromSanityObject(item.description) : ""
