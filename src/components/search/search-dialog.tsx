  ...
  const handleSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const searchResponse = await index.search<SearchHit>(value, {
        hitsPerPage: 10,
        attributesToRetrieve: ["title", "url", "type", "description", "image", "lastModified"],
        queryParameters: {
                        ...
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center justify-between">
                            <span className="font-medium text-fiery-red">
                              {typeof hit.title === 'string' ? hit.title : extractTextFromSanityObject(hit.title)}
                            </span>
                            {hit.type && (
                              <span className="rounded-full bg-neutral-200 px-2 py-1 text-xs dark:bg-neutral-700">
                                {typeof hit.type === 'string' ? hit.type.replace(/_/g, " ") : extractTextFromSanityObject(hit.type)}
                              </span>
                            )}
                          </div>
                          {descriptionText && (
